...
namespace ballistics_simulator
{
    ...
    CheckerGPIOController::CheckerGPIOController(std::string chipName, uint startLine, uint dropLine)
        : chipName(std::move(chipName)), startLine(startLine), dropLine(dropLine), request{}, chip{}
    {
    }

    auto CheckerGPIOController::init() -> void
    {
        auto chipPath = std::string("/dev/") + chipName;

        log() << "Open GPIO chip: " << chipPath << " Start line: " << startLine << " Drop line: " << dropLine;
    ...
    auto CheckerGPIOController::start() -> void
    {
    ...
    auto CheckerGPIOController::drop() -> void
    {
    ...
    CheckerGPIOController::~CheckerGPIOController()
    {
